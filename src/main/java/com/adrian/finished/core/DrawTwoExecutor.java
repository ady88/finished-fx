...
/**
 * AbilityExecutor for AbilitySpec.DRAW_TWO.
 *
 * Rules implemented (see game-abilities.json and AbilitySpec.DRAW_TWO):
 ...
 * - Draw two additional cards: from draw stack if available, otherwise from the end of Past
 * - Retrieved cards are appended to Present and marked fromDrawStack=false when coming from Past
 */
public final class DrawTwoExecutor implements AbilityExecutor {
    @Override
    public GameState apply(AbilityContext context) {
        if (context.ability() != AbilitySpec.DRAW_TWO) {
            throw new IllegalArgumentException("DrawTwoExecutor can only execute DRAW_TWO ability");
        }
        ...
        DrawHelpers.Result res = DrawHelpers.drawUpTo(2, s.drawStack(), s.past());
