...
This document describes the key API abstractions used by finished-core. It focuses on interfaces, enums, and records from the public API surface (no concrete engine implementations). These types primarily live in the finished-api dependency and are consumed by this module's executors.
...
Notes:
- Only API-level abstractions are shown. No implementation classes from finished-core are included in this diagram.
- Records are depicted as simple data holders for readability.
- finished-core plugs concrete executors onto `AbilityExecutor` to transform `GameState`, while adhering to these APIs from finished-api.
